...
def preprocess_card(roi: np.ndarray) -> np.ndarray:
    """扑克牌区预处理：padding, resize, gray+red fusion, CLAHE"""
    h, w = roi.shape[:2]
...
def main():
    ...
    cards = extract_cards(res_pub[0] if res_pub else [])
    for t, conf, box in cards:
        # 计算绝对坐标
        abs_pts = [[int(pt[0]/2 + x1 - 10), int(pt[1]/2 + y1 - 10)] for pt in box]
        result["publicCards"].append({"action": t, "box": abs_pts})
        cx = int(np.mean([p[0] for p in abs_pts]))
        cy = int(np.mean([p[1] for p in abs_pts]))
        cv2.putText(out, t, (cx, cy),
    ...
    cards = extract_cards(res_hand[0] if res_hand else [])
    for t, conf, box in cards:
        abs_pts = [[int(pt[0]/2 + x1 - 10), int(pt[1]/2 + y1 - 10)] for pt in box]
    ...
    roi_click_color = img[y1:y2, x1:x2]
    gray = cv2.cvtColor(roi_click_color, cv2.COLOR_BGR2GRAY)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
    roi_click = clahe.apply(gray)
